...
function updateHistoryViewport() {
  ...
  // Force layout recalculation after removing constraints.
  list.getBoundingClientRect();

  const computedStyle = window.getComputedStyle(list);
  const gapValue = parseFloat(computedStyle.rowGap || computedStyle.gap || '0') || 0;

  let visibleHeight = 0;
  for (let i = 0; i < 5; i += 1) {
    visibleHeight += entries[i].offsetHeight;
  }
  visibleHeight += gapValue * 4;
